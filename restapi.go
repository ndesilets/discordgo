...
func (s *Session) FormRequestWithBucketID(method, urlStr string, data FormWriter, bucketID string) (response []byte, err error) {
	...
	if err = data.Write(multipartWriter); err != nil {
		return
	}

	multipartWriter.Close()

	return s.request(method, urlStr, multipartWriter.FormDataContentType(), body.Bytes(), bucketID, 0)
}
...
// GuildStickerCreate creates a new Sticker
// guildID : The ID of a Guild.
...
// GuildStickerEdit modifies and returns updated Sticker
// guildID : The ID of a Guild.
